...
| session | `data/automod-session.ts` | `check-automod` (34) | dedupe within 1 s; slot within 2 s; late tail reported; a different screen opening ends the visit; the interface it came from carried as evidence |
...
| publish | `data/automod-publish.ts` | `check-strip-life` (46), `check-automod` (34) | the decision the controller cannot be gated on, driven directly: four known rows, a learned row, an unknown row, the Mods segment, idle |
